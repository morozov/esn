...
const term_codes_ansi:Ttermcodes=
      ...
      term_codes_xterm:Ttermcodes=
        (#$0E,                                              {enter_alt_charset_mode}
         ...
         { ESN: reverted to stock FPC 3.2.2 sequences for both
           cursor-visible variants. Trunk's `#27'[?25h'#27'[4 q'' and
           `#27'[?25h'#27'[2 q'' force a *steady* underline/block style
           (DECSCUSR). On macOS Terminal.app with user-preference blink
           enabled, this produces a visible cursor on top of whatever
           glyph is at (CursorX,CursorY). Stock 3.2.2 uses `[?12;25h`
           which only re-enables blink + visibility and leaves cursor
           style at the terminal default — so a blinking cursor lands
           in its OFF phase half the time and visually disappears. }
...
{$endif logging}
     ...
     decide_codepages;
{ ESN: removed kitty-protocol keyboard probe (#27'[>31u'). Stock
  FPC 3.2.2 Keyboard doesn't parse kitty-protocol replies, so the
  terminal's response byte stream would be fed to GetKeyEvent and
  returned as a spurious event. }
   end
  else
...
begin
{ ESN: removed matching `#27'[<u'` kitty-keys disable. }
  prepareDoneVideo;
  SetCursorType(crUnderLine);
{$ifdef linux}
  if Console=ttylinux then
