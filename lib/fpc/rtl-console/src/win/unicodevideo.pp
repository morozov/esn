...
unit UnicodeVideo;   { ESN: renamed from 'Video' to avoid shadowing }
{$ENDIF FPC_DOTTEDUNITS}
{ ESN: vendored upstream RTL has unused locals that -Sewn promotes
  to errors on Windows.  Suppress those notes for this unit only;
  our own code keeps strict checking via the project Makefile. }
{$WARN 5024 OFF}  { Parameter not used }
{$WARN 5025 OFF}  { Local variable not used }
{$WARN 5027 OFF}  { Local variable assigned but never used }
{$WARN 5028 OFF}  { Local typed const not used }
interface
...
begin
  ...
  if NoConsoleOnStart then
   ...
   begin
    ...
    if res then ;  { ESN: result deliberately ignored }
...
begin
  ...
  if SmallForce then
   begin
      ...
      { Mark East Asian Wide chars with COMMON_LVB_LEADING_BYTE /
        TRAILING_BYTE so the Windows console renders them spanning
        two cells even on terminals/fonts that don't auto-detect
        width.  Without this, wide chars render as one cell with the
        next cell visible as a separate glyph, breaking column
        alignment. }
      BufCounter := 0;
      LineCounter := 1;
      while LineCounter <= ScreenHeight do
      begin
        ColCounter := 1;
        while ColCounter <= ScreenWidth do
        begin
          eawWide := false;
          if ColCounter < ScreenWidth then
            eawWide := GetEastAsianWidth(
              UCS4Char(Ord(LineBuf[BufCounter].UnicodeChar))
            ) in [eawW, eawF];
          if eawWide then
          begin
            LineBuf[BufCounter].Attributes :=
              LineBuf[BufCounter].Attributes or COMMON_LVB_LEADING_BYTE;
            LineBuf[BufCounter + 1].UnicodeChar :=
              LineBuf[BufCounter].UnicodeChar;
            LineBuf[BufCounter + 1].Attributes :=
              (LineBuf[BufCounter].Attributes and $00FF)
              or COMMON_LVB_TRAILING_BYTE;
            Inc(BufCounter, 2);
            Inc(ColCounter, 2);
          end else
          begin
