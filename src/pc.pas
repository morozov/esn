...
Uses
     Crt, RV, Mouse,
     Vars, Palette, sn_Mem, Main, Main_Ovr,
...
s:='~`'+strr(i)+'~` file'+efiles(i,lang)+' with ~`'+changechar(extnum(strr(m)),' ',',')+'~` bytes';
s0:=s;
i:=19-(length(without(s0,'~`'))div 2); if i<0 then i:=0;
s:=space(i)+s0; s:=s+space(abs(38-CClen(s))); if CClen(s)>38 then delete(s,39+8,10);
...
s:='~`'+changechar(extnum(strr(disksize(ord(lp.pcnd[1])-64))),' ',',')+'~` total bytes on drive ~`'+lp.pcnd[1]+':~`';
s0:=s;
i:=19-(length(without(s0,'~`'))div 2); if i<0 then i:=0;
s:=space(i)+s0; s:=s+space(abs(38-CClen(s))); if CClen(s)>38 then delete(s,39+8,10);
...
s:='~`'+changechar(extnum(strr(m)),' ',',')+'~` Bytes memory for Navigator';
s0:=s;
i:=19-(length(without(s0,'~`'))div 2); if i<0 then i:=0;
s:=space(i)+s0; s:=s+space(abs(38-CClen(s))); if CClen(s)>38 then delete(s,39+8,10);
...
s:='~`'+strr(totalXMS)+'~`K free extended memory';
s0:=s;
i:=19-(length(without(s0,'~`'))div 2); if i<0 then i:=0;
s:=space(i)+s0; s:=s+space(abs(38-CClen(s))); if CClen(s)>38 then delete(s,39+8,10);
...
if i>0 then
 begin
  ...
  s:='~`'+strr(i)+'~` block'+eb(i,lang)+' selected in ~`'+strr(l)+'~` Hob file'+ewfiles(l,lang);
  s0:=s;
  i:=19-(length(without(s0,'~`'))div 2); if i<0 then i:=0;
  s:=space(i)+s0; s:=s+space(abs(38-CClen(s))); if CClen(s)>38 then delete(s,39+8,10);
...
if InsedOf(focus)>1 then
 Begin
  ...
  if wtype=_F5
  then
    s:='Copy ~`'+s+'~` file'+efiles(InsedOf(focus),eng)
  else
    s:='Remane or move ~`'+s+'~` file'+efiles(InsedOf(focus),eng);
 End;
...
Colour(pal.bkdInputNT,pal.txtdInputNT); curon;
 ...
 if not scanf_esc then
  begin
   ...
   if nospace(s)<>'' then TargetPath:=pcndOf(Focus)+(s);
