...
if nospace(name)<>'' then
 begin
 {$I-}
  stemp:='File '+name+'.scl'+' already exist.'+#255+' Overwrite?';
  if checkdirfile(p.pcnd+name+'.scl')=0 then if not cquestion(stemp,lang) then begin restscr; exit; end;
 ...
 {$I+}
  ...
  if i<>0 then
   begin
    errormessage('Error '+strr(i)+' while create SCL-file');
    exit;
   end;
  sclMakeImage:=true;
 end;
