...
Begin
 if p.trdDir^[ind].length>65533 then
  begin
   errormessage('Error! File length more than 65533 bytes');
   exit;
  end;
...
{============================================================================}
Function tapSave(var p:TPanel):boolean;
var buf:array[1..21] of byte; cs:byte; i:word; f:file;
Begin
tapSave:=false;
 if HobetaInfo.length>65533 then
  begin
   errormessage('Error! File length more than 65533 bytes');
...
if i<>0 then
 begin
  errormessage('Error '+strr(i)+' while create TAP-file');
  exit;
 end;
tapMakeImage:=true;
end;
